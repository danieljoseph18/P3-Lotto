...
contract BRRRaffle is ReentrancyGuard, IBRRRaffle, Ownable {
        ...
        Ownable(msg.sender)
    {
        ...
        randomGenerator = IRNG(_randomGeneratorAddress);
     ...
     */
    function closeLottery(uint256 _lotteryId) external override nonReentrant onlyOperator {
        if (_lotteries[_lotteryId].status != Status.Open) revert BRRRaffle_LotteryNotOpen();
    ...
    function changeRandomGenerator(address _randomGeneratorAddress) external onlyOwner {
        ...
        IRNG(_randomGeneratorAddress).requestRandomWords();

        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        IRNG(_randomGeneratorAddress).viewRandomResult();
